export default function onResize() {
  ...
  //Filter data by clicking on legend.
    ...
    legendItems
        ...
        .remove(); // don't need 'em
    legendItems
        .on('click', function(d) {
            const
                legendItem = d3.select(this), // clicked legend item
                selected = !legendItem.classed('selected'); // selected boolean
            legendItem
                .classed('selected', selected); // toggle selected class
            const
            ...
            legendItem
                .style(
                    {'background': selected
                        ? 'lightgray'
                        : 'white'}); // set background of legend items corresponding to selected statuses to light gray
            statusOptions
                        ...
                        chart.filters
                            ...
                            .forEach(filter => {
                                if (filtered === false && filter.val !== 'All')
                                    filtered = d[filter.col] !== filter.val || filter.val.indexOf(d[filter.col]) === -1;
                            });

                        return !filtered;
                    }); // define filtered data
            chart.filters
                .filter(filter => filter.col === 'Status')[0]
