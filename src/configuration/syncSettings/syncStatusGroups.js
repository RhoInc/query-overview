export default function syncStatusGroups(settings) {
    //age ranges
    settings.ageRanges = settings.age_cutoffs
        .map((d,i) => i > 0 ? [settings.age_cutoffs[i-1],d] : [0,d]);
    ...
    //age range colors
    let ageRangeColors = settings.age_range_colors.slice(settings.age_range_colors.length - settings.ageRanges.length);
    ...
    const defaultStatusGroups = [
        {
            ...
            label: 'Query Age',
            order: settings.ageRangeCategories
                .concat(
