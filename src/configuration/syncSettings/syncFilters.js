export default function syncFilters(settings) {
    //recency ranges
    settings.recencyRanges = settings.recency_cutoffs
        .map((d,i) => i > 0 ? [settings.recency_cutoffs[i-1],d] : [0,d]);
    settings.recencyRanges.push([settings.recency_cutoffs[settings.recency_cutoffs.length - 1],null]);

    //recency range categories
    settings.recencyRangeCategories = settings.recency_cutoffs.every(recency_range => recency_range%7 === 0)
        ? settings.recencyRanges
            .map((recencyRange,i) => (
    ...
    settings.status_groups.slice().reverse().forEach(status_group => {
