export default function defineQueryAgeCategories() {
    const queryAgeCategoryInput = this.controls.config.inputs
        .find(input => input.value_col === this.config.age_category_col);
    const queryAgeCategoryGroup = this.config.status_groups
        .find(age_category_group => age_category_group.value_col === this.config.age_category_col);
    const queryStatusOrder = this.config.status_groups
    ...
    const queryAgeCategoryOrder = Array.isArray(queryAgeCategoryGroup.order)
        ? queryAgeCategoryGroup.order.concat(
            d3.set(this.raw_data.map(d => d[this.config.age_category_col]))
                .values()
