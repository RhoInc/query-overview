export default function setYDomain() {
    ...
    this.y_dom = this.y_dom
        .filter(d => this.current_data.map(di => di.key).indexOf(d) > -1);
    ...
    if (this.config.cutoff !== 'All')
        this.y_dom = this.y_dom
            .filter((d,i) => i >= this.y_dom.length - this.config.cutoff);
}
