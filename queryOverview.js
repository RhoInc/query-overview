(function (global, factory) {
    ...
    function syncGroups(settings) {
        var defaultGroups = [{ value_col: settings.form_col, label: 'Form' }, { value_col: 'Form: Field', label: 'Form: Field' }, { value_col: settings.site_col, label: 'Site' }, { value_col: settings.marking_group_col, label: 'Marking Group' }, { value_col: settings.visit_col, label: 'Visit/Folder' }];
    ...
    function syncStatusGroups(settings) {
        ...
        settings.ageRangeCategories = settings.age_cutoffs.every(function (age_range) {
            ...
            return i < settings.ageRanges.length - 1 ? ageRange.map(function (days) {
                return days / 7;
            }).join('-') + ' wks' : '>' + ageRange[0] / 7 + ' wks';
        }) : settings.ageRanges.map(function (ageRange, i) {
            return i < settings.ageRanges.length - 1 ? ageRange.join('-') + ' days' : '>' + ageRange[0] + ' days';
        });
    ...
    function syncFilters(settings) {
        ...
        settings.recencyRanges.push([settings.recency_cutoffs[settings.recency_cutoffs.length - 1], null]);
        ...
        settings.recencyRangeCategories = settings.recency_cutoffs.every(function (recency_range) {
            ...
            return i < settings.recencyRanges.length - 1 ? recencyRange.map(function (days) {
                return days / 7;
            }).join('-') + ' weeks' : '>' + recencyRange[0] / 7 + ' weeks';
        }) : settings.recencyRanges.map(function (recencyRange, i) {
            return i < settings.recencyRanges.length - 1 ? recencyRange.join('-') + ' days' : '>' + recencyRange[0] + ' days';
        });
    ...
    function syncSettings(settings) {
        var syncedSettings = Object.assign({}, clone(settings), { clone: clone, arrayOfVariablesCheck: arrayOfVariablesCheck });
    ...
    function styles() {
      ...
      '.qo-control-grouping--filters {' + '    width: 20%;' + '    float: left;' + '    display: flex;' + '    flex-wrap: wrap;' + '    justify-content: space-evenly;' + '}', '.qo-subsetter {' + '    margin: 5px 2px !important;' + '    border-top: 1px solid #aaa;' + '    padding-top: 5px;' + '}', '.qo-subsetter .wc-control-label {' + '    margin: 0 5px 3px 0;' + '    text-align: center;' + '}', '.qo-select-all {' + '}', '.qo-subsetter .changer {' + '    margin: 0 auto;' + '}',

      /***--------------------------------------------------------------------------------------\
        Chart
      \--------------------------------------------------------------------------------------***/

      '.qo-component--chart {' + '    width: 58%;' + '    margin: 0 auto;' + '    position: relative;' + '}', '.qo-button--reset-chart {' + '    position: absolute;' + '    top: 0;' + '    left: 0;' + '    z-index: 2;' + '    width: 91px;' + '    padding: 3px 0;' + '}', '.qo-button--undo {' + '    position: absolute;' + '    bottom: 0;' + '    left: 0;' + '    z-index: 2;' + '    width: 91px;' + '    padding: 3px 0;' + '}', '.qo-component--chart .wc-chart {' + '    z-index: 1;' + '}', '.qo-component--chart .legend-title {' + '    cursor: help;' + '}', '.qo-component--chart .legend-item {' + '    cursor: pointer;' + '    border-radius: 4px;' + '    padding: 5px;' + '    padding-left: 8px;' + '    margin-right: 5px !important;' + '}', '.qo-footnote {' + '    width: 100%;' + '    text-align: center;' + '    font-style: italic;' + '}',

      /***--------------------------------------------------------------------------------------\
        Listing
      \--------------------------------------------------------------------------------------***/

      '.qo-component--listing {' + '    width: 100%;' + '}', '.qo-button--reset-listing {' + '    padding: 3px;' + '    margin: 10px 5px 10px 0;' + '}', '.qo-table-container {' + '    overflow-x: auto;' + '    width: 100%;' + '    transform: rotate(180deg);' + '    -webkit-transform: rotate(180deg); ' + '}', '.qo-table {' + '    width: 100%;' + '    transform: rotate(180deg);' + '    -webkit-transform: rotate(180deg); ' + '    display: table;' + '}', '.qo-table th {' + '    white-space: nowrap;' + '}', '.qo-table th,' + '.qo-table td {' + '    min-width: 100px;' + '    padding-right: 10px !important;' + '}'];
    ...
    function classControls() {
        this.controls.controlGroups = this.controls.wrap.selectAll('.control-group').attr('class', function (d) {
            return 'control-group qo-' + d.type + ' qo-' + d.type + '--' + d.label.toLowerCase().replace(/[^_a-zA-Z-]/g, '-');
        });
    }
    ...
    function addControlTooltips() {
        ...
        this.controls.controlGroups.each(function (d) {
            var tooltip = tooltips[d.label] || 'This ' + d.type + ' controls ' + (d.value_col || d.option || d.options.join(', ')) + '.';
            if (tooltips[d.label] === undefined) console.warn('The control labeled ' + d.label + ' does not have a curated tooltip. Defaulting to ' + tooltip + '.');
    ...
    function syncQueryAgeAndStatus(d, checked) {
        ...
        checkbox.attr('title', checked ? 'Deselect all ' + datum.label + ' options' : 'Select all ' + datum.label + ' options');
    ...
    function updateFilter(d, element) {
        ...
        //Update checkbox tooltip.
        checkbox.attr('title', checked ? 'Deselect all ' + d.label + ' options' : 'Select all ' + d.label + ' options');
    ...
    function addSelectAll() {
        ...
        .each(function (d) {
            ...
            var label = d3.select(this).html(d.label + ' <input class = "qo-select-all" type = "checkbox"></input>');

            //Add event listener to select-all checkbox.
            var checkbox = label.select('input').datum(d).attr('title', 'Deselect all ' + d.label + ' options').property('checked', true).on('click', function (di) {
    ...
    function updateSelectAll(d, selectedOptions) {
        ...
        if (d.multiple) {
            ...
            }).attr('title', checked ? 'Deselect all ' + d.label + ' options' : 'Select all ' + d.label + ' options').property('checked', checked);
        }
    }
    ...
    function updateFilterEventListeners() {
        ...
        this.controls.filters.selects.on('change', function (d) {
            ...
            if (['Query Age', 'Query Status'].indexOf(d.label) > -1) syncQueryAgeAndStatus$1.call(context, d, selectedOptions);
    ...
    function addResetButton() {
        ...
        this.resetButton = d3.select(this.div).insert('button', ':first-child').classed('qo-button qo-button--reset-chart', true).text('Reset chart').on('click', function () {
            ...
            _this.destroy();
            d3.select(_this.element).selectAll('*').remove();
    ...
    function setLeftMargin() {
        ...
        this.config.margin.left = Math.max(Math.max(7, d3.max(this.y_dom, function (d) {
            return d.length;
        })) * fontSize * 0.5 + fontSize * 1.5 * 1.5 + 6, 100);
    }
    ...
    function setChartHeight() {
        ...
        this.raw_height = this.filtered_data.length ? (+this.config.range_band + this.config.range_band * this.config.padding) * this.y_dom.length : 100;
    }

    function updateXAxisLabel() {
        d3.select('.x.axis').select('.axis-title').text(this.config.x.label + ' (' + String(d3.sum(this.current_data, function (d) {
            return d.total;
        })) + ')');
    }
    ...
    function annotateYAxisInfo() {
        ...
        if (this.y_dom.length < this.y_dom_length) {
            this.svg.append('text').attr({
                ...
                dy: 15,
                'text-anchor': 'end'
            }).style('cursor', 'help').text('and ' + (this.y_dom_length - this.y_dom.length) + ' more').on('mouseover', function () {
            ...
            }).append('title').text('The number of ' + this.config.y.label.toLowerCase() + 's can be changed with the Show First N Groups radio buttons.');
        }
    }
    ...
    function initListing() {
        ...
        var selectedData = d3.selectAll('rect.selected').data().flatMap(function (d) {
            return d.values.raw;
        });

        //Feed data from selected bars into listing.
        if (selectedData.length > 0) this.listing.init(selectedData);else this.listing.init(this.filtered_data);
    }
    ...
    function manualSort() {
        ...
        this.data.manually_sorted = this.data.raw.sort(function (a, b) {
            ...
            _this.sortable.order.forEach(function (item) {
                ...
                if (item.col !== context.initialSettings.age_col && item.col !== context.initialSettings.recency_col) {
                    if (order === 0) {
            ...
            this.thead_cells.on('click', function (d) {
                ...
                if (!sortItem) {
                    sortItem = {
                        col: col,
                        direction: 'ascending',
                        wrap: context.sortable.wrap.append('div').datum({ key: col }).classed('wc-button sort-box', true).text(header)
                    };
