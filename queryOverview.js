(function (global, factory) {
    ...
    function styles() {
      ...
      '.qo-component--listing {' + '    width: 100%;' + '}', '.qo-button--reset-listing {' + '    padding: 3px;' + '    margin: 10px 5px 10px 0;' + '}', '.qo-table-container {' + '    overflow-x: auto;' + '    width: 100%;' + '    transform: rotate(180deg);' + '    -webkit-transform: rotate(180deg); ' + '}', '.qo-table {' + '    width: 100%;' + '    transform: rotate(180deg);' + '    -webkit-transform: rotate(180deg); ' + '}'];
    ...
    function addSelectAll() {
        ...
        this.controls.filters.labels.filter(function (d) {
            ...
            var checkbox = label.select('input').datum(d).attr('title', 'Deselect all ' + d.label + ' options').property('checked', true).on('click', function (di) {
                ...
                checkbox.attr('title', checked ? 'Deselect all ' + di.label + ' options' : 'Select all ' + di.label + ' options');
    ...
    function syncQueryAgeAndStatus(d, selectedOptions) {
        ...
        var correspondingOptions = d3.merge(Object.keys(map).filter(function (key) {
    ...
    function updateFilterEventListeners() {
        ...
        this.controls.filters.selects.on('change', function (d) {
            ...
            if (['queryage', context.config.status_col].indexOf(d.value_col) > -1) syncQueryAgeAndStatus.call(context, d, selectedOptions);
            context.draw();
        });
    }
    ...
    function manualSort() {
        ...
        this.data.manually_sorted = this.data.raw.sort(function (a, b) {
            ...
            _this.sortable.order.forEach(function (item) {
                ...
                if (item.col !== context.chart.initialSettings.age_col && item.col !== context.chart.initialSettings.open_col) {
                    if (order === 0) {
