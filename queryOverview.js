(function (global, factory) {
    ...
    (global = global || self, global.queryOverview = factory(global.d3, global.webCharts));
    ...
    if (typeof Element !== 'undefined' && Element) Element.prototype.remove = function () {
      this.parentElement.removeChild(this);
    };
    if (typeof NodeList !== 'undefined' && NodeList) NodeList.prototype.remove = HTMLCollection.prototype.remove = function () {
    ...
    function rendererSettings() {
      return {
        ...
        age_range_colors: ['#ffffcc', '#ffeda0', '#fed976', '#feb24c', '#fd8d3c', '#fc4e2a', '#e31a1c', '#bd0026', '#800026'],
        //miscellany
    ...
    function syncStatusGroups(settings) {
      ...
      settings.ageRangeCategories = settings.age_cutoffs.every(function (age_range) {
        ...
        return i < settings.ageRanges.length - 1 ? "".concat(ageRange.map(function (days) {
          return days / 7;
        }).join('-'), " wks") : ">".concat(ageRange[0] / 7, " wks");
      }) : settings.ageRanges.map(function (ageRange, i) {
        return i < settings.ageRanges.length - 1 ? "".concat(ageRange.join('-'), " days") : ">".concat(ageRange[0], " days");
      }); //age range colors

      var ageRangeColors = settings.age_range_colors.slice(settings.age_range_colors.length - settings.ageRanges.length);
      settings.status_order.forEach(function (status, i) {
        if (settings.age_statuses.indexOf(status) < 0) ageRangeColors.push(settings.status_colors[i]);
      ...
      var defaultStatusGroups = [{
        ...
        label: 'Query Age',
        order: settings.ageRangeCategories.concat(settings.status_order.filter(function (status) {
    ...
    function syncWebchartsSettings(settings) {
      ...
      settings.legend.order = settings.status_group.order ? settings.status_group.order.slice() : null;
    }
    ...
    function syncFilters(settings) {
      ...
      settings.recencyRanges.push([settings.recency_cutoffs[settings.recency_cutoffs.length - 1], null]); //recency range categories
      ...
      settings.recencyRangeCategories = settings.recency_cutoffs.every(function (recency_range) {
        ...
        return i < settings.recencyRanges.length - 1 ? "".concat(recencyRange.map(function (days) {
          return days / 7;
        }).join('-'), " weeks") : ">".concat(recencyRange[0] / 7, " weeks");
      }) : settings.recencyRanges.map(function (recencyRange, i) {
        return i < settings.recencyRanges.length - 1 ? "".concat(recencyRange.join('-'), " days") : ">".concat(recencyRange[0], " days");
      }); //default filters
      ...
      settings.status_groups.slice().reverse().forEach(function (status_group) {
    ...
    function layout(element) {
      var containers = {
        main: d3.select(element).append('div').classed('query-overview', true)
      };
      containers.topRow = containers.main.append('div').classed('qo-row qo-row--top', true);
      containers.controls = containers.topRow.append('div').classed('qo-component qo-component--controls', true);
      containers.chart = containers.controls.append('div').classed('qo-component qo-component--chart', true);
      containers.bottomRow = containers.main.append('div').classed('qo-row qo-row--bottom', true);
      containers.listing = containers.bottomRow.append('div').classed('qo-component qo-component--listing', true);
      return containers;
    }
    ...
    function styles(document) {
      ...
      /***--------------------------------------------------------------------------------------\
        Controls
      \--------------------------------------------------------------------------------------***/
      '.qo-component--controls {' + '    width: 100%;' + '}', '.qo-component--controls .wc-controls {' + '    margin-bottom: 0;' + '}', '.qo-control-grouping {' + '    display: inline-block;' + '}', '.qo-button {' + '    float: left;' + '    display: block;' + '}', '.qo-control-grouping--label,' + '.wc-control-label {' + '    cursor: help;' + '    margin-bottom: 3px;' + '}', '.qo-control-grouping--label {' + '    text-align: center;' + '    width: 100%;' + '    font-size: 24px;' + '    border-bottom: 2px solid #aaa;' + '}', '.span-description {' + '    display: none !important;' + '}',
      /****---------------------------------------------------------------------------------\
        Other controls
      \---------------------------------------------------------------------------------****/
      '.qo-control-grouping--other-controls {' + '    width: 20%;' + '    float: right;' + '}', '.qo-control-grouping--other-controls .control-group {' + '    width: 100%;' + '    margin-bottom: 15px;' + '}', '.qo-control-grouping--other-controls .control-group:nth-child(n+3) {' + '    border-top: 1px solid #aaa;' + '}', '.qo-control-grouping--other-controls .control-group .wc-control-label {' + '    text-align: center;' + '    font-size: 110%;' + '}', //dropdowns
      '.qo-dropdown {' + '}', '.qo-dropdown .wc-control-label {' + '}', '.qo-dropdown .changer {' + '    margin: 0 auto;' + '}', //radio buttons
      '.qo-radio {' + '    display: flex !important;' + '    justify-content: center;' + '    flex-wrap: wrap;' + '}', '.qo-radio .wc-control-label {' + '    width: 100%;' + '}', '.qo-radio .radio {' + '    margin-top: 0 !important;' + '}', //checkboxes
      '.qo-checkbox {' + '    display: flex !important;' + '    justify-content: center;' + '}', '.qo-checkbox .wc-control-label {' + '    margin-right: 5px;' + '}', '.qo-checkbox .changer {' + '    margin-top: 5px !important;' + '}',
      /****---------------------------------------------------------------------------------\
        Filters
      \---------------------------------------------------------------------------------****/
      ...
      /***--------------------------------------------------------------------------------------\
        Chart
      \--------------------------------------------------------------------------------------***/
      '.qo-component--chart {' + '    width: 58%;' + '    margin: 0 auto;' + '    position: relative;' + '}', '.qo-button--reset-chart {' + '    position: absolute;' + '    top: 0;' + '    left: 0;' + '    z-index: 2;' + '    width: 91px;' + '    padding: 3px 0;' + '}', '.qo-button--undo {' + '    position: absolute;' + '    bottom: 0;' + '    left: 0;' + '    z-index: 2;' + '    width: 91px;' + '    padding: 3px 0;' + '}', '.qo-component--chart .wc-chart {' + '    z-index: 1;' + '}', '.qo-component--chart .legend-title {' + '    cursor: help;' + '}', '.qo-component--chart .legend-item {' + '    cursor: pointer;' + '    border-radius: 4px;' + '    padding: 5px;' + '    padding-left: 8px;' + '    margin-right: 5px !important;' + '}', '.qo-component--chart .y.axis .tick text {' + '    font-family: "Lucida Console", "Courier New", "Monospace";' + '    font-size: 14px;' + '}', '.qo-footnote {' + '    width: 100%;' + '    text-align: center;' + '    font-style: italic;' + '}',
      /***--------------------------------------------------------------------------------------\
        Listing
      \--------------------------------------------------------------------------------------***/
      '.qo-component--listing {' + '    width: 100%;' + '}', '.qo-button--reset-listing {' + '    padding: 3px;' + '    margin: 10px 5px 10px 0;' + '}', '.qo-table-container {' + '    overflow-x: auto;' + '    width: 100%;' + '    transform: rotate(180deg);' + '    -webkit-transform: rotate(180deg); ' + '}', '.qo-table {' + '    width: 100%;' + '    transform: rotate(180deg);' + '    -webkit-transform: rotate(180deg); ' + '    display: table;' + '}', '.qo-table th {' + '    white-space: nowrap;' + '}', '.qo-table th,' + '.qo-table td {' + '    min-width: 100px;' + '    padding-right: 10px !important;' + '}']; //Attach styles to DOM.
    ...
    function defineNewVariables() {
      ...
      this.raw_data.forEach(function (d) {
        ...
        if (_this.config.age_statuses.indexOf(d[_this.config.status_col]) < 0) d[queryAgeCol] = d[_this.config.status_col];else {
          var age = +d[_this.config.age_col];

          _this.config.ageRanges.forEach(function (ageRange, i) {
            if (i === 0 && ageRange[0] <= age && age <= ageRange[1]) d[queryAgeCol] = _this.config.ageRangeCategories[i];else if (i === _this.config.ageRanges.length - 1 && ageRange[0] < age) d[queryAgeCol] = _this.config.ageRangeCategories[i];else if (ageRange[0] < age && age <= ageRange[1]) d[queryAgeCol] = _this.config.ageRangeCategories[i];
          });
        ...
        if (d.hasOwnProperty(_this.config.recency_category_col)) {
          ...
          _this.config.recencyRanges.forEach(function (recencyRange, i) {
            if (i === 0 && recencyRange[0] <= recency && recency <= recencyRange[1]) d[queryRecencyCol] = _this.config.recencyRangeCategories[i];else if (i === _this.config.recencyRanges.length - 1 && recencyRange[0] < recency) d[queryRecencyCol] = _this.config.recencyRangeCategories[i];else if (recencyRange[0] < recency && recency <= recencyRange[1]) d[queryRecencyCol] = _this.config.recencyRangeCategories[i];
          });
        }
      });
    }
    ...
    function defineQueryStatusSet() {
      ...
      var queryStatusOrder = Array.isArray(queryStatusGroup.order) && queryStatusGroup.order.length ? queryStatusGroup.order.concat(d3.set(this.raw_data.map(function (d) {
        return d[_this.config.status_col];
      })).values().filter(function (value) {
        return queryStatusGroup.order.indexOf(value) < 0;
      }).sort()) : d3.set(this.raw_data.map(function (d) {
        return d[_this.config.status_col];
      })).values().sort();
    ...
    function defineQueryRecencySet() {
      ...
      if (this.raw_data[0].hasOwnProperty(this.config.recency_category_col)) {
        queryRecencyInput.values = d3.set(this.raw_data.map(function (d) {
      ...
      } else if (this.raw_data[0].hasOwnProperty(this.config.recency_col)) queryRecencyInput.values = this.config.recencyRangeCategories;else this.controls.config.inputs.splice(this.controls.config.inputs.findIndex(function (input) {
        return input.value_col === 'queryrecency';
      }), 1);
    }
    ...
    function defineMaps() {
      ...
      this.maps = {
        querystatus: d3.nest().key(function (d) {
          return d[_this.config.status_col];
        }).rollup(function (d) {
          return d3.set(d.map(function (di) {
            return di.queryage;
          })).values();
        }).map(this.raw_data),
        queryage: d3.nest().key(function (d) {
          return d.queryage;
        }).rollup(function (d) {
          return d3.set(d.map(function (di) {
    ...
    function removeInvalidControls() {
      ...
      var updated_inputs = this.controls.config.inputs.filter(function (d) {
        return d.derive_source ? d.derive_source in context.raw_data[0] : d.value_col ? d.value_col in context.raw_data[0] : true;
      });
    ...
    function classControls() {
      this.controls.controlGroups = this.controls.wrap.selectAll('.control-group').attr('class', function (d) {
        return "control-group qo-".concat(d.type, " qo-").concat(d.type, "--").concat(d.label.toLowerCase().replace(/[^_a-zA-Z-]/g, '-'));
      });
    }
    ...
    function groupControls() {
      ...
      this.controls.filters = {
        container: this.controls.wrap.insert('div', '.qo-subsetter').classed('qo-control-grouping qo-control-grouping--filters', true)
      };
      this.controls.filters.container.append('div').classed('qo-control-grouping--label', true).attr('title', 'Filters subset the data underlying the chart and listing.\nHover over filter labels to view more information about them.').text('Filters');
      ...
      this.controls.otherControls = {
        container: this.controls.wrap.insert('div', ':first-child').classed('qo-control-grouping qo-control-grouping--other-controls', true)
      };
      this.controls.otherControls.label = this.controls.otherControls.container.append('div').classed('qo-control-grouping--label', true).attr('title', 'Controls alter the display of the chart.\nHover over control labels to view more information about them.').text('Controls');
      this.controls.otherControls.controlGroups = this.controls.wrap.selectAll('.control-group:not(.qo-subsetter)');
    ...
    function addControlTooltips() {
      var tooltips = {
        //other controls
        'Group by': 'Controls the variable with which the queries are grouped; each group is plotted along the vertical axis of the chart.',
        'Status Group': 'Controls the variable with which the bars are subdivided.',
        'Bar Arrangement': 'Controls the layout of the status groups.\n- stacked: status groups are plotted side-by-side horizontally\n- grouped: status groups are plotted side-by-side vertically',
        'Show First N Groups': 'Controls the number of groups displayed on the vertical axis.',
        'Order Groups Alphabetically?': 'Controls the order of the groups; uncheck to sort groups by magnitude (largest to smallest number of queries) instead of alphabetically.',
        //filters
        'Query Age': 'Open queries are broken down into how long they have been open. All other queries are classified by status (answered, closed, cancelled).',
        'Query Status': 'Open: site has not responded to the issue\nAnswered: site has responded to issue; DM needs to review\nClosed: issue resolved\nCancelled: query cancelled by DM',
        'Query Recency': 'Number of days a query has been open, regardless of its current status (applies only to queries opened in the past 30 days)',
        Form: 'CRF page abbreviation; hover over the abbreviation in the chart to see its full name.',
        Site: 'Name or ID of site',
        'Marking Group': 'Entity that opened the query',
        'Visit/Folder': 'Visit/folder abbreviation; hover over the visit/folder abbreviation in the chart to see the full name.'
      };
      this.controls.controlGroups.each(function (d) {
        var tooltip = tooltips[d.label] || "This ".concat(d.type, " controls ").concat(d.value_col || d.option || d.options.join(', '), ".");
        if (tooltips[d.label] === undefined) console.warn("The control labeled ".concat(d.label, " does not have a curated tooltip. Defaulting to ").concat(tooltip, "."));
        d3.select(this).selectAll('.wc-control-label').attr('title', tooltip);
      });
    }

    function updateGroupByOptions() {
      var context = this;
      this.controls.wrap.selectAll('.control-group select').filter(function (d) {
    ...
    function addGroupByHighlight() {
      ...
      this.controls.otherControls.controlGroups.filter(function (d) {
        return d.label === 'Group by';
    ...
    function checkInitialStatusGroup() {
      ...
      }).selectAll('.radio').selectAll('.changer').property('checked', function (d) {
    ...
    function customizeMultiSelects() {
      var context = this;
      this.controls.wrap.selectAll('.control-group select').filter(function () {
        return this.multiple;
      }).attr('title', 'Hold the CTRL key while clicking to select or deselect a single option.').property('size', function () {
        ...
        d3.select(this).selectAll('option').sort(function (a, b) {
    ...
    function syncQueryAgeAndStatus(d, checked) {
      var checkbox = d.label === 'Query Age' ? this.controls.filters.checkboxes.filter(function (di) {
      ...
      checkbox.attr('title', checked ? "Deselect all ".concat(datum.label, " options") : "Select all ".concat(datum.label, " options")); //Update filter object.

      var filter = this.filters.find(function (filter) {
        return filter.col === datum.value_col;
      });
      if (checked) filter.val = filter.choices;else filter.val = [];
    }
    ...
    function updateFilter(d, element) {
      ...
      checkbox.attr('title', checked ? "Deselect all ".concat(d.label, " options") : "Select all ".concat(d.label, " options")); //Update filter object.

      var filter = this.filters.find(function (filter) {
        return filter.col === d.value_col;
      });
      if (checked) filter.val = filter.choices;else filter.val = []; //Sync query age and status filters.
    ...
    function addSelectAll() {
      var context = this;
      this.controls.filters.labels.filter(function (d) {
        return d.multiple;
      }) // only multi-selects
      .each(function (d) {
        //Add checkbox to filter label.
        var label = d3.select(this).html("".concat(d.label, " <input class = \"qo-select-all\" type = \"checkbox\"></input>")); //Add event listener to select-all checkbox.

        var checkbox = label.select('input').datum(d).attr('title', "Deselect all ".concat(d.label, " options")).property('checked', true).on('click', function (di) {
    ...
    function updateSelectAll(d, selectedOptions) {
      ...
      if (d.multiple) {
        ...
        var checkbox = this.controls.filters.checkboxes.filter(function (di) {
          return di.value_col === d.value_col;
        }).attr('title', checked ? "Deselect all ".concat(d.label, " options") : "Select all ".concat(d.label, " options")).property('checked', checked);
      }
    }
    ...
    function syncQueryAgeAndStatus$1(d, selectedOptions) {
      ...
      var correspondingOptions = d3.set(d3.merge(Object.keys(map).filter(function (key) {
    ...
    function updateFilterEventListeners() {
      ...
      this.controls.filters.selects.on('change', function (d) {
        ...
        if (['Query Age', 'Query Status'].indexOf(d.label) > -1) syncQueryAgeAndStatus$1.call(context, d, selectedOptions); //Redraw.
    ...
    function sortQueryRecencyOptions() {
      this.controls.filters.selects.filter(function (d) {
    ...
    function addResetButton() {
      ...
      this.resetButton = d3.select(this.div).insert('button', ':first-child').classed('qo-button qo-button--reset-chart', true).text('Reset chart').on('click', function () {
    ...
    function addFootnotes() {
      this.footnotes = {
        barClick: this.wrap.append('div').classed('qo-footnote qo-footnote--bar-click', true).text('Click one or more bars to view the underlying data in the listing below.'),
        deselectBars: this.wrap.append('div').classed('qo-footnote qo-footnote--deselect-bars', true).text('Click in the white area to deselect all bars.')
      };
    }
    ...
    function updateStratification() {
      var statusGroup = this.controls.wrap.selectAll('.qo-radio--status-group').selectAll('.radio').filter(function () {
    ...
    function highlightSelectedOptions() {
      ...
      this.controls.wrap.selectAll('.control-group select').filter(function () {
        ...
        d3.select(this).selectAll('option').property('selected', function (di) {
    ...
    function setLeftMargin() {
      ...
      this.config.maxYAxisTickLabelLength = Math.max(7, d3.max(this.y_dom, function (d) {
        return d.length;
      })) * this.config.fontSize * 0.5 + this.config.fontSize * 1.5 * 1.5 + 6; //Set the left margin to the minimum of one quarter of the container width and the maximum number of characters in the y-axis tick labels (minimum: 100 pixels).

      this.config.margin.left = Math.min(this.config.maxYAxisTickWidth, Math.max(this.config.maxYAxisTickLabelLength, 100));
    }
    ...
    function setYDomain() {
      ...
      this.y_dom = this.y_dom.filter(function (d) {
        return _this.current_data.map(function (di) {
          return di.key;
        }).indexOf(d) > -1;
    ...
    function setChartHeight() {
      ...
      this.raw_height = this.filtered_data.length ? Math.max((+this.config.range_band + this.config.range_band * this.config.padding) * this.y_dom.length, 100) : 100;
    }

    function updateXAxisLabel() {
      this.svg.select('.x.axis').select('.axis-title').text(this.config.x.label + ' (' + String(d3.sum(this.current_data, function (d) {
        return d.total;
      })) + ')');
    }
    ...
    function legendFilter() {
      ...
      var legendItems = this.wrap.selectAll('.legend-item').classed('selected', function (d) {
    ...
    function addYAxisTickTooltips() {
      ...
      var yAxisTicks = this.svg.selectAll('.y.axis .tick').filter(function (form) {
      ...
      switch (this.config.y.column) {
        case this.config.form_col:
          yAxisTicks.append('title').text(function (form) {
            return "Form: ".concat(_this.raw_data.filter(function (d) {
        ...
        case 'Form: Field':
          yAxisTicks.style('cursor', 'help').append('title').text(function (field) {
            var datum = _this.raw_data.filter(function (d) {
              return d['Form: Field'] === field;
            })[0];

            return "Form: ".concat(datum[_this.config.formDescription_col] || datum[_this.config.form_col], "\nField: ").concat(datum[_this.config.fieldDescription_col] || datum[_this.config.field_col]);
          });
          break;
    ...
    function addYAxisTickClick() {
      ...
      if (this.config.y.column === this.config.form_col) {
        ...
        yLabels.style('cursor', 'pointer').on('click', function (yLabel) {
          ...
          groupByControl.select('.wc-control-label').style({
            'font-weight': 'bold',
            'text-decoration': 'underline',
            color: 'red'
          }).transition().delay(3000).style({
            'font-weight': 'normal',
            'text-decoration': 'none',
            color: 'black'
          });
          ...
          formFilter.select('.wc-control-label').style({
            'font-weight': 'bold',
            'text-decoration': 'underline',
            color: 'red'
          }).transition().delay(3000).style({
          ...
          filter.val = yLabel;
          updateSelectAll.call(_this, _this.controls.filters.controlGroups.filter(function (d) {
            return d.value_col === _this.config.form_col;
          }).datum(), [yLabel]); //Redraw chart and listing.
          //Update Group by control.
          ...
          _this.svg.select('.y.axis .axis-title').style({
            'font-weight': 'bold',
            'text-decoration': 'underline',
            fill: 'red'
          }).transition().delay(3000).style({
    ...
    function annotateYAxisInfo() {
      ...
      if (this.y_dom.length < this.y_dom_length) {
        this.svg.append('text').attr({
          ...
          dy: 15,
          'text-anchor': 'end'
        }).style('cursor', 'help').text("and ".concat(this.y_dom_length - this.y_dom.length, " more")).on('mouseover', function () {
        ...
        }).append('title').text("The number of ".concat(this.config.y.label.toLowerCase(), "s can be changed with the Show First N Groups radio buttons."));
      }
    }

    function truncateYAxisTickLabels() {
      var yAxisLabelHeight = this.svg.select('.y.axis .axis-title').node().getBBox().height;
      var yAxisTickLabelSpace = this.config.maxYAxisTickWidth - yAxisLabelHeight - 4;
      if (this.config.maxYAxisTickLabelLength > yAxisTickLabelSpace) this.svg.selectAll('.y.axis .tick text').each(function (d) {
    ...
    function hideBars() {
      ...
      this.svg.select('g.bar-supergroup').selectAll('g.bar-group').attr('display', function (d) {
          ...
          if (context.y_dom.indexOf(d.key) > -1) {
            d.values.forEach(function (di) {
              context.svg.append('text').classed('number-of-queries', true).attr({
                ...
                y: context.y(d.key) + context.y.rangeBand() * (context.config.color_dom.length - 1 - context.config.color_dom.indexOf(di.key)) / context.config.color_dom.length,
                dx: '0.25em',
                dy: '1em'
    ...
    function resetListing() {
      ...
      var selectedData = this.svg.selectAll('rect.selected').data().flatMap(function (d) {
        return d.values.raw;
      });
    ...
    function addNoDataIndicator() {
      ...
      if (this.filtered_data.length === 0) this.svg.append('text').classed('qo-no-data', true).attr({
        x: this.plot_width / 2,
    ...
    function applyVariableMetadata() {
      ...
      var variableMetadata = [{
        ...
        description: 'Number of days between query open date and data extraction date, regardless of query status.'
      }, {
        variable: this.chart.initialSettings.recency_category_col,
        label: 'Query Recency',
        description: 'Number of days by category between query open date and data extraction date, regardless of query status. Categories include last 7, 14, and 30 days.'
      }, {
        ...
        description: 'Number of days between query open date and data extraction date, query response date, or query resolution date for open, answered, and closed/cancelled queries, respectively.'
      }, {
        variable: 'queryage',
        label: 'Query Age',
        description: 'Number of days by category between query open date and data extraction date, query response date, or query resolution date for open, answered, and closed/cancelled queries, respectively.'
      }, {
    ...
    function addResetButton$1() {
      ...
      this.resetButton = this.wrap.insert('button', ':first-child').classed('qo-button qo-button--reset-listing', true).text('Reset listing').on('click', function () {
    ...
    function addTableContainer() {
      ...
      this.tableContainer = this.wrap.append('div').classed('qo-table-container', true).node();
    ...
    function manualSort() {
      ...
      this.data.manually_sorted = this.data.raw.sort(function (a, b) {
        ...
        _this.sortable.order.forEach(function (item) {
          ...
          if (item.col !== context.chart.initialSettings.age_col && item.col !== context.chart.initialSettings.recency_col) {
            if (order === 0) {
        ...
        this.thead_cells.on('click', function (d) {
          ...
          if (!sortItem) {
            sortItem = {
              col: col,
              direction: 'ascending',
              wrap: context.sortable.wrap.append('div').datum({
                key: col
          ...
          context.sortable.order.forEach(function (item, i) {
            item.wrap.on('click', function (d) {
              ...
              context.sortable.wrap.select('.instruction').classed('hidden', context.sortable.order.length); //Redraw table.
    ...
    function truncateCellText() {
      ...
      if (this.data.raw.length) this.tbody.selectAll('td').attr('title', function (d) {
        return d.text;
      }).filter(function (d) {
        return d.text.length > _this.chart.initialSettings.truncation_cutoff;
      }).text(function (d) {
        return "".concat(d.text.substring(0, _this.chart.initialSettings.truncation_cutoff), "...");
      });
    }
    ...
    function queryOverview$1() {
      ...
      var syncedControlInputs = configuration.syncControlInputs(configuration.controlInputs, syncedSettings); //Layout and styles
