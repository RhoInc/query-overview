...
function removeInvalidControls() {
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (d) {
        return d.derive_source ? !(d.derive_source in context.raw_data[0]) : d.value_col ? !(d.value_col in context.raw_data[0]) : false;
    }).remove();
}
...
function defineStyles() {
    var styles = ['.wc-table .table-container {' + '    overflow-x: auto;' + '    width : 100%;' + '    transform:  rotate(180deg);' + ' -webkit-transform:rotate(180deg); ' + '}', '.wc-table table {' + '    transform:  rotate(180deg);' + '  -webkit-transform:rotate(180deg); ' + '}'];
